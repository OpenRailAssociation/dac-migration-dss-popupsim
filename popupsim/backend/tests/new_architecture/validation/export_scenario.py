...
def export_scenario_to_json(scenario, output_dir: Path) -> None:
    ...
    # Workshops
    workshops = [
        {"id": w.id, "track": w.track, "retrofit_stations": w.retrofit_stations}
        for w in scenario.workshops
    ]
    (output_dir / "workshops.json").write_text(json.dumps(workshops, indent=2))

    # Routes
    routes = [
        {"id": r.id, "duration": r.duration, "track_sequence": r.track_sequence}
    ...
    for train in scenario.trains:
        arrival_time = (
            train.arrival_time.isoformat()
            if hasattr(train.arrival_time, "isoformat")
            else train.arrival_time
        )
        trains.append(
            {
