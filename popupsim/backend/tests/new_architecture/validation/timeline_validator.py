...
def validate_timeline_from_docstring(
    result: Any, test_func: Any, analytics_context: Any = None
) -> None:
    ...
    events = (
        analytics_context.get_metrics().get("event_history", [])
...
def _validate_wagon(
    ...
    if track_match:
        ...
        found = any(
            ...
            and (
                ...
                or (
                    hasattr(e, "parking_area_id")
                    and e.parking_area_id == expected_track
                )
            )
            for t, e in events
        )
    ...
    if status_name == "RETROFITTING" and "retrofit_start" in rest:
        ...
        found = any(
            abs(t - time) < 0.5
            and type(e).__name__
            in ("RetrofitCompletedEvent", "WagonRetrofitCompletedEvent")
            and hasattr(e, "wagon_id")
            and e.wagon_id == wagon_id
            for t, e in events
        )
