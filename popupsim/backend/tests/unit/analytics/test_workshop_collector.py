...
def test_workshop_idle_time() -> None:
    ...
    events = [
        WorkshopUtilizationChangedEvent(
            EventId.generate(), Timestamp.from_simulation_time(0.0), 'WS001', 0.0, 0
        ),
        WorkshopUtilizationChangedEvent(
            EventId.generate(), Timestamp.from_simulation_time(30.0), 'WS001', 50.0, 1
        )
    ]
...
def test_multiple_workshops() -> None:
    ...
    events = [
        ...
        WorkshopUtilizationChangedEvent(
            EventId.generate(), Timestamp.from_simulation_time(5.0), 'WS002', 25.0, 1
        )
    ]
...
def test_simulation_end_event() -> None:
    ...
    event = WorkshopUtilizationChangedEvent(
        EventId.generate(), Timestamp.from_simulation_time(0.0), 'WS001', 50.0, 1
    )
    collector.record_event(event)
