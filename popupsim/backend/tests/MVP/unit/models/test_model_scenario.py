...
class TestScenario:
    ...
    def test_scenario_config_with_file_from_fixture(
        self, test_scenario_json_path: Path
    ) -> None:
        """
    ...
    def test_track_selection_strategy_validation(self) -> None:
        ...
        """
        from popupsim.backend.src.MVP.configuration.domain.models.scenario import (
    ...
    def test_scenario_config_json_serialization(self) -> None:
        ...
        assert str(parsed["start_date"]).startswith(
            str(datetime(2024, 1, 1, tzinfo=UTC)).split(" ")[0]
        )
        assert str(parsed["end_date"]).startswith(
            str(datetime(2024, 1, 10, tzinfo=UTC)).split(" ")[0]
        )
        # Compare dates ignoring timezone differences
        parsed_start = datetime.fromisoformat(parsed.get("start_date")).replace(
            tzinfo=None
        )
