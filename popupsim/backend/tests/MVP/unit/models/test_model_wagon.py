...
class TestWagon:
    ...
    def test_wagon_valid_lengths(self, length_value: float) -> None:
        ...
        wagon = Wagon(
            id="W_PARAM", length=length_value, is_loaded=True, needs_retrofit=False
        )

        assert wagon.length == length_value

    @pytest.mark.parametrize(
        ("is_loaded", "needs_retrofit"),
        [(True, True), (True, False), (False, True), (False, False)],
    )
    def test_wagon_boolean_combinations(
        self, is_loaded: bool, needs_retrofit: bool
    ) -> None:
