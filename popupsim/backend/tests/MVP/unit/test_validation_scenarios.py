...
from MVP.simulation.application.simulation_orchestrator import SimulationOrchestrator
from MVP.simulation.infrastructure.engines.simpy_engine_adapter import (
    SimPyEngineAdapter,
)
from MVP.workshop_operations.application.workshop_context import (
...
def test_two_wagons_two_stations() -> None:
    ...
    """
    scenario = create_minimal_scenario_with_dtos(
        num_wagons=2, num_stations=2, retrofit_time=10.0
    )
    engine = SimPyEngineAdapter.create()
    workshop_context = WorkshopOperationsContext(scenario)
    orchestrator = SimulationOrchestrator(engine, scenario)
...
def test_four_wagons_two_stations() -> None:
    ...
    """
    scenario = create_minimal_scenario_with_dtos(
        num_wagons=4, num_stations=2, retrofit_time=10.0
    )
    engine = SimPyEngineAdapter.create()
    workshop_context = WorkshopOperationsContext(scenario)
    orchestrator = SimulationOrchestrator(engine, scenario)
...
def test_six_wagons_two_workshops() -> None:
    ...
    assert ws1_stations[0].wagons_completed == 2, (
        f"WS1[0] expected 2, got {ws1_stations[0].wagons_completed}"
    )
    assert ws1_stations[1].wagons_completed == 2, (
        f"WS1[1] expected 2, got {ws1_stations[1].wagons_completed}"
    )

    ws2_stations = popup_sim.workshop_capacity.stations["WS2"]
    assert ws2_stations[0].wagons_completed == 1, (
        f"WS2[0] expected 1, got {ws2_stations[0].wagons_completed}"
    )
    assert ws2_stations[1].wagons_completed == 1, (
        f"WS2[1] expected 1, got {ws2_stations[1].wagons_completed}"
    )
...
def test_seven_wagons_two_workshops() -> None:
    ...
    processed_wagons = [
        w for w in popup_sim.wagons if w.retrofit_start_time is not None
    ]
    assert len(processed_wagons) == 7, (
        f"Expected 7 wagons processed, got {len(processed_wagons)}"
    )
