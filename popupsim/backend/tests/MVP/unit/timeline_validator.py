...
from popupsim.backend.src.MVP.workshop_operations.domain.entities.locomotive import (
    LocoStatus,
)
...
def _validate_loco(  # noqa: PLR0913
    ...
    loco = next(
        (
            locomotive
            for locomotive in popup_sim.shunting_operations.locomotives_collection
            if locomotive.id == loco_id
        ),
        None,
    )
    ...
    if end_time and status == LocoStatus.MOVING:
        ...
        if route_match:
            ...
            from popupsim.backend.src.MVP.workshop_operations.infrastructure.routing.route_finder import (
                find_route,
            )
...
def _validate_wagon(
    popup_sim: Any, wagon_id: str, time: float, _status_name: str, rest: str
) -> None:
    """Validate wagon state at specific time."""
    wagon = next((w for w in popup_sim.wagons if w.id == wagon_id), None)
    ...
    # Check track - temporarily disabled due to timeline validator checking final states instead of intermediate states
    # if 'track=' in rest:
...
def validate_loco_timeline(loco: Any, timeline_spec: str) -> None:
    ...
    for i, ((exp_time, exp_status), (act_time, act_status)) in enumerate(
        zip(transitions, history, strict=True)
    ):
        assert act_time == exp_time, f"[{i}] Expected t={exp_time}, got t={act_time}"
        assert act_status == exp_status, (
            f"[{i}] Expected {exp_status}, got {act_status}"
        )
