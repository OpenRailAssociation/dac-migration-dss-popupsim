...
from popupsim.backend.src.MVP.analytics.domain.services.kpi_calculator import (
    KPICalculator,
)
from popupsim.backend.src.MVP.configuration.domain.models.scenario import Scenario
from popupsim.backend.src.MVP.workshop_operations.domain.entities.wagon import (
    Wagon,
    WagonStatus,
)
from popupsim.backend.src.MVP.workshop_operations.domain.entities.workshop import (
    Workshop,
)
...
@pytest.mark.asyncio
async def test_calculate_wagons_per_hour(
    sample_scenario: Scenario, sample_wagons: list[Wagon], sample_metrics: dict
) -> None:
...
async def test_calculate_utilization(
    ...
    sample_workshops: list[Workshop],
    sample_metrics: dict,
) -> None:
...
@pytest.mark.asyncio
async def test_identify_bottleneck_high_rejection(
    sample_scenario: Scenario, sample_metrics: dict
) -> None:
    ...
    rejection_bottlenecks = [
        b for b in result.bottlenecks if "rejection" in b.description.lower()
    ]
    assert len(rejection_bottlenecks) > 0


@pytest.mark.asyncio
async def test_identify_bottleneck_high_utilization(
    sample_scenario: Scenario, sample_metrics: dict
) -> None:
...
@pytest.mark.asyncio
async def test_avg_waiting_time_calculation(
    sample_scenario: Scenario, sample_metrics: dict
) -> None:
...
@pytest.mark.asyncio
async def test_no_wagons_scenario(
    sample_scenario: Scenario, sample_metrics: dict
) -> None:
...
@pytest.mark.asyncio
async def test_no_workshops_scenario(
    sample_scenario: Scenario, sample_wagons: list[Wagon], sample_metrics: dict
) -> None:
...
async def test_empty_metrics(
    sample_scenario: Scenario, sample_wagons: list[Wagon]
) -> None:
...
@pytest.mark.asyncio
async def test_wagons_without_waiting_time(
    sample_scenario: Scenario, sample_metrics: dict
) -> None:
...
@pytest.mark.asyncio
async def test_scenario_id_in_result(
    sample_scenario: Scenario, sample_wagons: list[Wagon], sample_metrics: dict
) -> None:
...
async def test_bottleneck_severity_levels(
    ...
    if result.bottlenecks:
        assert all(
            b.severity in ["low", "medium", "high", "critical"]
