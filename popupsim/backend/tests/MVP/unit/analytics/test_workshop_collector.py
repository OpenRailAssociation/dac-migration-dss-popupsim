...
from popupsim.backend.src.MVP.analytics.domain.collectors.workshop_collector import (
    WorkshopCollector,
)
...
def test_record_workshop_station_occupied() -> None:
    ...
    event = WorkshopUtilizationChangedEvent.create(
...
def test_idle_time() -> None:
    ...
    events = [
        WorkshopUtilizationChangedEvent.create(
            Timestamp.from_simulation_time(0.0),
            workshop_id="WS001",
...
def test_simulation_end_event() -> None:
    ...
    end_event = WorkshopUtilizationChangedEvent.create(
