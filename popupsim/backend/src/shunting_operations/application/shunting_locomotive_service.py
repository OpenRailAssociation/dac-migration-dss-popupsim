...
class ShuntingLocomotiveService(LocomotiveService):
    ...
    def allocate(self, popupsim: Any) -> Generator[Any, Any, Locomotive]:
        ...
        """
        shunting_loco = yield from self.shunting_service.allocate_shunting_locomotive(popupsim)
    ...
    def release(self, popupsim: Any, loco: Locomotive) -> Generator[Any]:
        ...
        if shunting_loco:
            yield from self.shunting_service.release_shunting_locomotive(popupsim, shunting_loco)
    ...
    def move(self, popupsim: Any, loco: Locomotive, from_track: str, to_track: str) -> Generator[Any]:
        ...
        shunting_loco = self._shunting_locomotives.get(loco.id)
        if shunting_loco:
            yield from self.shunting_service.execute_shunting_move(popupsim, shunting_loco, from_track, to_track)
    ...
    def couple_wagons(
        ...
        shunting_loco = self._shunting_locomotives.get(loco.id)
        if shunting_loco:
            yield from self.shunting_service.execute_coupling(popupsim, shunting_loco, wagon_count, coupler_type)
    ...
    def decouple_wagons(
        ...
        shunting_loco = self._shunting_locomotives.get(loco.id)
        if shunting_loco:
            yield from self.shunting_service.execute_decoupling(popupsim, shunting_loco, wagon_count, coupler_type)
