...
from ...domain.entities.shunting_operation import ShuntingOperation
from ...domain.services.shunting_plan_validator import ShuntingPlanValidationError
from ...domain.services.shunting_plan_validator import ShuntingPlanValidator
...
class ShuntingValidationPipeline:
    ...
    def validate_domain_objects(
        ...
        try:
            ...
            ShuntingPlanValidator.validate_locomotive_consistency(locomotive_id, operations)
    ...
    def _validate_syntax(self, data: dict[str, Any], result: ValidationResult) -> None:
        ...
        if not isinstance(data.get('plan_id'), str):
            result.add_error('plan_id must be string', field_name='plan_id', category=ValidationCategory.SYNTAX)

        if not isinstance(data.get('locomotive_id'), str):
            result.add_error(
                'locomotive_id must be string', field_name='locomotive_id', category=ValidationCategory.SYNTAX
            )

        if not isinstance(data.get('steps'), list):
            result.add_error('steps must be list', field_name='steps', category=ValidationCategory.SYNTAX)

    def _validate_semantic(self, data: dict[str, Any], result: ValidationResult) -> None:
        """Layer 2: Business rules validation."""
        steps = data.get('steps', [])
        if len(steps) == 0:
            result.add_error('Plan must have at least one step', category=ValidationCategory.SEMANTIC)

    def _validate_integrity(self, data: dict[str, Any], result: ValidationResult) -> None:
        """Layer 3: Cross-reference validation."""
        # Check sequence ordering
        steps = data.get('steps', [])
        sequences = [step.get('sequence', -1) for step in steps if isinstance(step, dict)]
        if sequences != sorted(sequences):
            result.add_error('Steps must be in sequence order', category=ValidationCategory.INTEGRITY)

    def _validate_feasibility(self, data: dict[str, Any], result: ValidationResult) -> None:
        """Layer 4: Operational constraints."""
        steps = data.get('steps', [])
        if len(steps) > 50:
            result.add_warning('Large plans may impact performance', category=ValidationCategory.FEASIBILITY)
