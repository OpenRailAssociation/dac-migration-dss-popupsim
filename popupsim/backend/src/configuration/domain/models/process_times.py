...
class ProcessTimes(BaseModel):
    ...
    train_to_hump_delay: timedelta = Field(
        default=timedelta(minutes=10.0), description='Time from train arrival to first wagon at hump'
    )
    wagon_hump_interval: timedelta = Field(
        default=timedelta(minutes=2.0), description='Time between wagons passing through hump'
    )
    # Screw coupler times (before retrofit)
    screw_coupling_time: timedelta = Field(
        default=timedelta(minutes=1.0), description='Time for one screw coupling operation'
    )
    screw_decoupling_time: timedelta = Field(
        default=timedelta(minutes=1.0), description='Time for one screw decoupling operation'
    )

    # DAC coupler times (after retrofit)
    dac_coupling_time: timedelta = Field(
        default=timedelta(minutes=0.5), description='Time for one DAC coupling operation'
    )
    dac_decoupling_time: timedelta = Field(
        default=timedelta(minutes=0.5), description='Time for one DAC decoupling operation'
    )

    wagon_move_to_next_station: timedelta = Field(
        default=timedelta(minutes=0.5), description='Time to move wagons to next station'
    )
    ...
    wagon_coupling_retrofitted_time: timedelta = Field(
        default=timedelta(minutes=2.0), description='Legacy: Time to couple retrofitted wagon'
    )
    wagon_retrofit_time: timedelta = Field(
        default=timedelta(minutes=60.0), description='Time to retrofit one wagon at a station'
    )
    loco_parking_delay: timedelta = Field(
        default=timedelta(minutes=0.0), description='Time locomotive waits at parking before next trip'
    )

    @field_validator('*', mode='before')
    @classmethod
    def convert_float_to_timedelta(cls, v: float | timedelta | int) -> timedelta:
    ...
    def get_coupling_ticks(self, coupler_type: str) -> float:
        ...
        return timedelta_to_sim_ticks(self.get_coupling_time(coupler_type))

    def get_decoupling_ticks(self, coupler_type: str) -> float:
        """Get decoupling time in SimPy ticks."""
        return timedelta_to_sim_ticks(self.get_decoupling_time(coupler_type))
