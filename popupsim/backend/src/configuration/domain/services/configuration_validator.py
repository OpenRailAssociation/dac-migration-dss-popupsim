...
class ConfigurationValidator:  # pylint: disable=too-few-public-methods
    ...
    def _validate_scenario_basic(self, scenario: Scenario) -> ValidationResult:
        ...
        if not scenario.id or not scenario.id.strip():
            ...
            result.add_error('Scenario ID must contain only letters, numbers, hyphens, and underscores', 'id')
    ...
    def _validate_dates(self, scenario: Scenario) -> ValidationResult:
        ...
        if scenario.end_date <= scenario.start_date:
            result.add_error(
                f'End date ({scenario.end_date}) must be after start date ({scenario.start_date})', 'end_date'
            )

        duration = (scenario.end_date - scenario.start_date).days
        if duration > 365:
            result.add_warning(f'Simulation duration of {duration} days may impact performance', 'end_date')
        elif duration < 1:
            result.add_error(f'Simulation duration must be at least 1 day. Current: {duration} days', 'end_date')
            ...
            for i, train in enumerate(scenario.trains):
                ...
                if hasattr(train, 'arrival_time') and hasattr(train, 'departure_time'):
                    try:
                        ...
                        result.add_error(f'Train {train.id} has invalid time format', f'trains[{i}].arrival_time')
    ...
    def _validate_required_data(self, scenario: Scenario) -> ValidationResult:
        ...
        if not scenario.locomotives:
            result.add_error('Scenario must have at least one locomotive', 'locomotives')
