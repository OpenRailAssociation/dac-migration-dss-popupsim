...
class JsonDataSourceAdapter(DataSourcePort):
    ...
    def load_scenario(self, source_identifier: str | Path) -> ScenarioInputDTO:
        ...
        if not json_file.exists():
            raise DataSourceError(f'JSON file not found: {json_file}')

        try:
            with json_file.open('r', encoding='utf-8') as f:
                json.load(f)  # Validate JSON format
        except (json.JSONDecodeError, OSError) as e:
            raise DataSourceError(f'Failed to load JSON file: {e}') from e
    ...
    def _convert_scenario_to_dto(self, scenario: Any) -> ScenarioInputDTO:
        ...
        start_date = (
            scenario.start_date.isoformat() if hasattr(scenario.start_date, 'isoformat') else str(scenario.start_date)
        )
