...
class ConfigurationService:
    ...
    def load_scenario(self, path: str | Path) -> dict[str, Any]:
        ...
        if not file_path.exists():
            raise ConfigurationError(_('Scenario configuration file not found: %(path)s', path=str(file_path)))

        logger.info('Loading scenario configuration', translate=True, file_path=str(file_path))
        ...
        try:
            ...
            if missing_fields:
                raise ConfigurationError(
                    _(
                        'Missing required fields in %(path)s: %(missing)s. Found fields: %(found)s',
                        path=str(file_path),
                        missing=', '.join(missing_fields),
                        found=', '.join(data.keys()),
                    )
                )

            logger.info('Scenario loaded successfully', translate=True, scenario_id=data.get('scenario_id'))
            return data  # type: ignore[no-any-return]

        except json.JSONDecodeError as e:
            error_msg = _(
                'Invalid JSON syntax in %(path)s at line %(line)d, column %(col)d: %(msg)s. '
    ...
    def load_and_validate_scenario(self, path: str | Path) -> dict[str, Any]:
        ...
        if not train_schedule_file:
            raise ConfigurationError(_('Missing train_schedule_file in scenario configuration'))

        train_schedule_path = config_dir / train_schedule_file
        if not train_schedule_path.exists():
            raise ConfigurationError(_('Train schedule file not found: %(path)s', path=str(train_schedule_path)))

        logger.info('Referenced files validated', translate=True, scenario_id=scenario_data.get('scenario_id'))
    ...
    def _read_and_validate_train_schedule_csv(self, file_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            raise ConfigurationError(_('Train schedule file is empty')) from err
        except (pd.errors.ParserError, TypeError) as err:
            raise ConfigurationError(
                _('Error parsing CSV file %(path)s: %(error)s', path=str(file_path), error=str(err))
            ) from err

        # Validate that the result is actually a DataFrame
        if not isinstance(df, pd.DataFrame):
            raise ConfigurationError(_('Loaded object is not a pandas DataFrame'))

        if df.empty:
            raise ConfigurationError(_('Train schedule file is empty'))
        required_columns = [
            'train_id',
            'arrival_date',
            'arrival_time',
        ...
        if missing_columns:
            raise ConfigurationError(
                _(
                    'Missing required columns in %(path)s: %(missing)s. Found columns: %(found)s',
                    path=str(file_path),
        ...
        if not pd.api.types.is_datetime64_any_dtype(df['arrival_date']):
            raise ConfigurationError(_("Column 'arrival_date' must be in YYYY-MM-DD format."))
        ...
        if not invalid_times.empty:
            raise ConfigurationError(
                _(
                    "Column 'arrival_time' must be in HH:MM format. Invalid values: %(values)s",
                    values=', '.join(map(str, invalid_times['arrival_time'].unique())),
                )
            )
        return df
    ...
    def _create_wagons_from_group(self, group: pd.DataFrame) -> list[Wagon]:
        ...
        for _idx, row in group.iterrows():
            try:
                ...
                raise ConfigurationError(
                    _(
                        'Validation failed for wagon in train %(train_id)s in %(wagon_id)s:\n%(details)s',
                        train_id=row['train_id'],
                        wagon_id=row['wagon_id'],
                        details='  • ' + chr(10).join('  • ' + detail for detail in error_details),
                    )
                ) from err
        return wagons
    ...
    def _parse_arrival_time(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            raise ConfigurationError(
                _('Invalid time format in arrival_time. Expected HH:MM format: %(error)s', error=str(err))
            ) from err
    ...
    def _check_duplicate_wagons(self, df: pd.DataFrame) -> None:
        ...
        if len(duplicate_wagons) > 0:
            raise ConfigurationError(_('Duplicate wagon IDs found: %(ids)s', ids=', '.join(duplicate_wagons)))
    ...
    def _validate_train_consistency(self, train_id: str, group: pd.DataFrame) -> None:
        ...
        if unique_dates > 1 or unique_times > 1:
            raise ConfigurationError(
                _('Train %(train_id)s has inconsistent arrival date/time across wagons', train_id=train_id)
            )
    ...
    def _handle_train_validation_error(self, train_id: str, err: ValidationError) -> None:
        ...
        raise ConfigurationError(
            _(
                'Validation failed for train arrival %(train_id)s:\n%(details)s',
                train_id=train_id,
                details='  • ' + chr(10).join('  • ' + detail for detail in error_details),
            )
        ) from err
    ...
    def load_train_schedule(self, file_path: str | Path) -> list[Train]:
        ...
        if not file_path.exists():
            raise ConfigurationError(_('Train schedule file not found: %(path)s', path=str(file_path)))
        logger.info('Loading train schedule', translate=True, file_path=str(file_path))
        try:
            df = self._read_and_validate_train_schedule_csv(file_path)
            train_arrivals = self._create_train_arrivals(df)
            logger.info(
                'Train schedule loaded successfully',
                translate=True,
                train_count=len(train_arrivals),
                wagon_count=len(df),
                file_path=str(file_path),
            )
            ...
            raise ConfigurationError(
                _(
                    'Unexpected error loading train schedule from %(path)s: %(error)s',
                    path=str(file_path),
                    error=str(err),
                )
            ) from err
    ...
    def _create_workshop_tracks_from_dataframe(self, df: pd.DataFrame) -> list[WorkshopTrack]:
        ...
        for _idx, row in df.iterrows():
            try:
                ...
                raise ConfigurationError(
                    _(
                        'Validation failed for track %(track_id)s:\n%(details)s',
                ...
                raise ConfigurationError(
                    _('Invalid data type for track %(track_id)s: %(error)s', track_id=row['track_id'], error=str(err))
                ) from err
        return tracks
    ...
    def _read_and_validate_workshop_tracks_csv(self, file_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            raise ConfigurationError(_('Workshop tracks file is empty')) from err
        ...
        if df.empty:
            raise ConfigurationError(_('Workshop tracks file is empty'))
        ...
        if len(duplicate_ids) > 0:
            raise ConfigurationError(_('Duplicate track IDs found: %(ids)s', ids=', '.join(duplicate_ids)))
    ...
    def _create_workshop_from_tracks(self, tracks: list[WorkshopTrack]) -> Workshop:
        ...
        try:
            ...
            raise ConfigurationError(
                _(
                    'Validation failed for workshop configuration:\n%(details)s',
                    details='  • ' + chr(10).join('  • ' + detail for detail in error_details),
                )
            ) from err
        except Exception as err:
            raise ConfigurationError(
                _('Unexpected error creating workshop from tracks: %(error)s', error=str(err))
            ) from err
    ...
    def load_workshop_tracks(self, file_path: str | Path) -> Workshop:
        ...
        file_path = Path(file_path)
        if not file_path.exists():
            raise ConfigurationError(_('Workshop tracks file not found: %(path)s', path=str(file_path)))

        logger.info('Loading workshop tracks', translate=True, file_path=str(file_path))
        ...
        try:
            ...
            logger.info(
                'Workshop tracks loaded successfully', translate=True, track_count=len(tracks), file_path=str(file_path)
            )
            return workshop
            ...
            raise ConfigurationError(
                _(
                    'Unexpected error loading workshop tracks from %(path)s: %(error)s',
                    path=str(file_path),
                    error=str(err),
                )
            ) from err
    ...
    def _validate_scenario_dates(self, scenario_data: dict[str, Any]) -> tuple[date, date]:
        ...
        if not scenario_start_str or not scenario_end_str:
            raise ConfigurationError(_('Missing start_date or end_date in scenario configuration'))
        ...
        try:
            ...
            raise ConfigurationError(_('Invalid date format in scenario configuration: %(error)s', error=str(e))) from e
    ...
    def _validate_train_dates_in_range(self, trains: list[Train], scenario_start: date, scenario_end: date) -> None:
        ...
        if out_of_range_trains:
            raise ConfigurationError(
                _(
                    'Train arrivals outside scenario date range (%(start)s to %(end)s): %(trains)s',
                    start=str(scenario_start),
                    end=str(scenario_end),
                    trains=', '.join(out_of_range_trains),
                )
            )
    ...
    def _build_scenario_config(
        ...
        if not scenario_id:
            raise ConfigurationError(_('Missing scenario_id in scenario configuration'))
    ...
    def load_complete_scenario(self, path: str | Path) -> tuple[ScenarioConfig, ValidationResult]:
        ...
        logger.info('Complete scenario loaded successfully', translate=True, scenario_id=config.scenario_id)
