...
class WagonPickupCoordinator:  # pylint: disable=too-few-public-methods
    ...
    def pickup_wagons_to_retrofit(self) -> Generator:
        ...
        while True:
            collection_track_id, collection_wagons = yield from self._wait_for_wagons_ready()
            ...
            yield from self._deliver_to_retrofit_tracks(collection_track_id, wagons_by_retrofit)
    ...
    def _wait_for_wagons_ready(self) -> Generator[None, None, tuple[str, list[Wagon]]]:
        ...
        while True:
            ...
            wagons_by_track = self.orchestrator.wagon_selector.filter_selected_wagons(self.orchestrator.wagons_queue)
            if wagons_by_track:
    ...
    def _signal_wagons_ready(self, wagons_by_retrofit: dict[str, list[Wagon]]) -> Generator:
        """Signal wagons ready for workshop processing."""
        for retrofit_track_id, retrofit_wagons in wagons_by_retrofit.items():
            yield from self._distribute_wagons_to_workshops(retrofit_track_id, retrofit_wagons)

    def _distribute_wagons_to_workshops(self, retrofit_track_id: str, wagons: list[Wagon]) -> Generator:
        """Distribute wagons to workshops based on available capacity."""
        capacity_claims = {w.track: 0 for w in self.orchestrator.workshops_queue}
        ...
        while remaining:
            best_workshop = max(
                self.orchestrator.workshops_queue,
                key=lambda w: self.orchestrator.workshop_capacity.get_available_stations(w.track)
                - capacity_claims[w.track],
            )

            available = (
                self.orchestrator.workshop_capacity.get_available_stations(best_workshop.track)
                - capacity_claims[best_workshop.track]
            )
            if available <= 0:
                best_workshop = self.orchestrator.workshops_queue[0]
                batch = remaining
                remaining = []
            else:
            ...
            for wagon in batch:
                ...
                yield from self.orchestrator.put_wagon_for_station(best_workshop.track, retrofit_track_id, wagon)
