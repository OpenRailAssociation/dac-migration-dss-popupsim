...
from MVP.configuration.application.dtos.track_input_dto import (
    TrackInputDTO,
)
from MVP.configuration.application.dtos.train_input_dto import (
    TrainInputDTO,
)
from MVP.configuration.application.dtos.wagon_input_dto import (
    WagonInputDTO,
)
from MVP.configuration.application.dtos.workshop_input_dto import (
    WorkshopInputDTO,
)
from MVP.configuration.domain.models.process_times import (
    ProcessTimes,
)
from MVP.configuration.domain.models.scenario import (
    LocoDeliveryStrategy,
    Scenario,
    TrackSelectionStrategy,
)
from MVP.configuration.domain.models.topology import Topology
from MVP.configuration.domain.services.scenario_validator import (
    ScenarioValidator,
)
...
class ScenarioBuilder:
    ...
    def __load_locomotives(self) -> None:
        ...
        if not locomotives_file:
            msg = "Missing locomotives file reference in scenario configuration"
        ...
        if not locomotives_path.exists():
            msg = f"Locomotives file not found: {locomotives_path}"
            raise BuilderError(msg)
        ...
        try:
            if isinstance(self.scenario, Scenario):
                ...
                locomotive_list: list[dict[str, str]] = locomotive_data.get(
                    "locomotives"
                )
                self.scenario.locomotives = [
                    LocomotiveInputDTO(**data) for data in locomotive_list
                ]

        except json.JSONDecodeError as e:
            msg = f"Invalid JSON format in {locomotives_path}: {e!s}"
            raise BuilderError(msg) from e
        except Exception as e:
            msg = f"Failed to load locomotives from {locomotives_path}: {e!s}"
            raise BuilderError(msg) from e
    ...
    def __load_routes(self, scenario_dto: ScenarioInputDTO) -> None:
        ...
        """
        routes_file: str | None = scenario_dto.routes_file or self.references.get(
            "routes"
        )

        if not routes_file:
            msg = "Missing routes_file in scenario configuration"
            raise BuilderError(msg)
        ...
        if not routes_path.exists():
            msg = f"Routes file not found: {routes_path}"
            raise BuilderError(msg)
        ...
        try:
            ...
            msg = f"Failed to load routes from {routes_path}: {e!s}"
            raise BuilderError(msg) from e
    ...
    def __load_topology(self) -> None:
        ...
        if not topology_file:
            msg = "Missing topology file reference in scenario configuration"
            raise BuilderError(msg)
        ...
        if not topology_path.exists():
            msg = f"Topology file not found: {topology_path}"
            raise BuilderError(msg)
        ...
        try:
            ...
            msg = f"Failed to load topology from {topology_path}: {e!s}"
            raise BuilderError(msg) from e
    ...
    def __load_process_times(self) -> None:
        ...
        if not process_times_file:
            msg = "Missing process_times file reference in scenario configuration"
            raise BuilderError(msg)
        ...
        if not process_times_path.exists():
            msg = f"Process times file not found: {process_times_path}"
            raise BuilderError(msg)

        try:
            if isinstance(self.scenario, Scenario):
                self.scenario.process_times = ProcessTimes.load_from_file(
                    process_times_path
                )

        except Exception as e:
            msg = f"Failed to load process times from {process_times_path}: {e!s}"
            raise BuilderError(msg) from e
    ...
    def __load_tracks(self, scenario_dto: ScenarioInputDTO) -> None:
        ...
        tracks_file: str | None = (
            scenario_dto.workshop_tracks_file or self.references.get("tracks")
        )

        if not tracks_file:
            msg = "Missing workshop_tracks_file in scenario configuration"
            raise BuilderError(msg)
        ...
        if not tracks_path.exists():
            msg = f"Tracks file not found: {tracks_path}"
            raise BuilderError(msg)
        ...
        try:
            ...
            msg = f"Failed to load tracks from {tracks_path}: {e!s}"
            raise BuilderError(msg) from e
    ...
    def __load_trains(self, scenario_dto: ScenarioInputDTO) -> None:  # pylint: disable=too-many-locals
        ...
        """
        trains_file: str | None = (
            scenario_dto.train_schedule_file or self.references.get("trains")
        )

        if not trains_file:
            msg = "Missing train_schedule_file in scenario configuration"
            raise BuilderError(msg)
        ...
        if not trains_path.exists():
            msg = f"Trains file not found: {trains_path}"
            raise BuilderError(msg)
        ...
        try:
            if isinstance(self.scenario, Scenario):
                ...
                for train_id, group in df.groupby("train_id"):
                    ...
                    for _, row in group.iterrows():
                        wagon_dto = WagonInputDTO(
                            id=str(
                                row.get(
                                    "wagon_id",
                                    f"{train_id}_wagon_{len(wagon_dtos) + 1}",
                                )
                            ),
                    ...
                    train_dto = TrainInputDTO(
                        ...
                        locomotive_id=str(
                            first_row.get("locomotive_id", "default_loco")
                        ),
            ...
            msg = f"Failed to load trains from {trains_path}: {e!s}"
            raise BuilderError(msg) from e
    ...
    def __load_workshops(self) -> None:
        ...
        if not workshops_file:
            msg = "Missing workshops file reference in scenario configuration"
            raise BuilderError(msg)
        ...
        if not workshops_path.exists():
            msg = f"Workshops file not found: {workshops_path}"
            raise BuilderError(msg)
        ...
        try:
            if isinstance(self.scenario, Scenario):
                ...
                if "workshops" not in workshops_data:
                    msg = f"No workshops found in {workshops_path}"
                    raise BuilderError(msg)
            ...
            msg = f"Invalid JSON format in {workshops_path}: {e!s}"
            raise BuilderError(msg) from e
        except Exception as e:
            msg = f"Failed to load workshops from {workshops_path}: {e!s}"
            raise BuilderError(msg) from e
    ...
    def __find_scenario_in_path(self, path: Path) -> None:
        ...
        if path.is_dir():
            ...
            file_path = (
                test_file_path if test_file_path.exists() else path / SCENARIOFILE
            )
        ...
        if not file_path.exists():
            msg = f"Scenario models file not found: {file_path}"
            raise BuilderError(msg)
    ...
    def build(self) -> Scenario:
        ...
        self.scenario = Scenario(
            ...
            track_selection_strategy=scenario_dto.track_selection_strategy
            or TrackSelectionStrategy.LEAST_OCCUPIED,
            retrofit_selection_strategy=(
                scenario_dto.retrofit_selection_strategy
                or TrackSelectionStrategy.LEAST_OCCUPIED
            ),
            loco_delivery_strategy=scenario_dto.loco_delivery_strategy
            or LocoDeliveryStrategy.RETURN_TO_PARKING,
        )
        ...
        if isinstance(self.scenario, Scenario):
            ...
            if not validation_result.is_valid:
                ...
                if validation_result.has_errors():
                    error_messages = [
                        str(issue) for issue in validation_result.get_errors()
                    ]
                    msg = f"Scenario validation failed: {'\n'.join(error_messages)}"
                    raise BuilderError(msg)
                logger.warning("Scenario has validation warnings but will proceed")
        else:
            msg = "Scenario could not be loaded properly."
            raise BuilderError(msg)
