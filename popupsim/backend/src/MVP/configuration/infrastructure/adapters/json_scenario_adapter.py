...
class JsonScenarioAdapter(ScenarioPort):
    ...
    def _process_json_data(self, data: dict) -> ScenarioInputDTO:
        ...
        if "trains" in data:
            trains = [
                TrainInputDTO(
                    ...
                    wagons=[
                        WagonInputDTO(**wagon) for wagon in train.get("wagons", [])
                    ],
                )
        ...
        routes = [
            RouteInputDTO(
                id=r.get("id"),
                track_sequence=r.get("path", r.get("track_sequence", [])),
                duration=r.get("duration"),
            )
            for r in data.get("routes", [])
        ]
        locomotives = [
            LocomotiveInputDTO(**loco) for loco in data.get("locomotives", [])
        ]
