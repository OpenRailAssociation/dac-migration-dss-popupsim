...
class CsvDataSourceAdapter(DataSourcePort):
    ...
    def _load_scenario_metadata(self, csv_dir: Path) -> dict[str, Any]:
        ...
        if not scenario_file.exists():
            msg = f"scenario.csv not found in {csv_dir}"
            raise DataSourceError(msg)
    ...
    def _load_wagons_for_train(
        self, csv_dir: Path, train_id: str
    ) -> list[WagonInputDTO]:
        ...
        train_wagons = (
            df[df["train_id"] == train_id] if "train_id" in df.columns else df
        )
