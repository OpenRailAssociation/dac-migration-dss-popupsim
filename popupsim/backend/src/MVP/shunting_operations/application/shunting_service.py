...
from MVP.analytics.domain.events.locomotive_events import (
    LocomotiveStatusChangeEvent,
)
from MVP.analytics.domain.value_objects.timestamp import Timestamp
from MVP.shunting_operations.domain.entities.shunting_locomotive import (
    CapacityLimits,
    MovementState,
    ShuntingLocomotive,
    ShuntingStatus,
...
from MVP.workshop_operations.infrastructure.routing.route_finder import (
    find_route,
)
...
class ShuntingService(ABC):
    ...
    @abstractmethod
    def allocate_shunting_locomotive(
        self, popupsim: Any
    ) -> Generator[Any, Any, ShuntingLocomotive]:
    ...
    @abstractmethod
    def release_shunting_locomotive(
        self, popupsim: Any, loco: ShuntingLocomotive
    ) -> Generator[Any]:
...
class DefaultShuntingService(ShuntingService):
    ...
    def __init__(
        ...
        self.default_capacity = CapacityLimits(
            default_wagon_capacity, default_weight_tons, default_length_m
        )
    ...
    def execute_shunting_move(
        ...
        # Emit locomotive event for metrics collection
        event = LocomotiveStatusChangeEvent.create(
            timestamp=Timestamp.from_simulation_time(start_time),
            locomotive_id=loco.id,
            status=LocoStatus.MOVING.value,
        )
        popupsim.metrics.record_event(event)
        ...
        if duration > 0:
            ...
            logger.info(
                "✓ SHUNTING: Loco %s at %s (no travel needed)", loco.id, to_track
            )
    ...
    def execute_coupling(
        ...
        if not loco.can_couple_wagon(coupler_type):
            logger.warning(
                "⚠️ SHUNTING: Loco %s cannot couple %s wagons (incompatible coupler)",
                loco.id,
                coupler_type.value,
            )
        ...
        time_per_coupling = process_times.get_coupling_ticks(coupler_type.value)
    ...
    def execute_decoupling(
        ...
        process_times = popupsim.scenario.process_times
        if coupler_type:
            time_per_coupling = process_times.get_decoupling_ticks(coupler_type.value)
        else:
            time_per_coupling = to_ticks(process_times.screw_decoupling_time)
        decoupling_time = wagon_count * time_per_coupling

        # Only change state if decoupling takes time
        if decoupling_time > 0:
            current_time = popupsim.sim.current_time()
            loco.base_locomotive.record_status_change(
                current_time, LocoStatus.DECOUPLING
            )
            ...
            logger.info(
                "🔓 SHUNTING: Loco %s decoupling %d wagons for %.1f min",
                loco.id,
                wagon_count,
                decoupling_time,
            )
