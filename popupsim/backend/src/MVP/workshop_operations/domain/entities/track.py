...
class Track(BaseModel):
    ...
    valid_from: datetime | None = Field(
        default=None, description="Start of track validity period"
    )

    valid_to: datetime | None = Field(
        default=None, description="End of track validity period"
    )

    _total_length: float | None = None  # Cached total length
    ...
    def validate_track_by_type_value(self) -> Self:
        ...
        try:
            ...
            msg = f"Invalid track type: {self.type}"
    ...
    def validate_edges(self) -> Self:
        ...
        """
        if not self.edges:
            msg = f"Edges list cannot be empty for track {self.id}"
            raise ValueError(msg)
        return self

    def set_total_length(self, total_length: float) -> None:
        """Set cached total length.

        Parameters
        ----------
        total_length : float
            Total length of all edges in this track.
        """
        self._total_length = total_length

    def get_total_length(self) -> float | None:
        """Get cached total length.

        Returns
        -------
        float | None
            Cached total length or None if not yet calculated.
        """
