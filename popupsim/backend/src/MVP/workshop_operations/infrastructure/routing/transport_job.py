...
def execute_transport_job(
    popupsim: "OrchestrationContext", job: TransportJob, loco_service: LocomotiveService
) -> Generator[Any, Any]:
    """Execute a transport job: allocate resource, move, couple, transport, decouple, release.

    Parameters
    ----------
    popupsim : OrchestrationContext
        The orchestration context instance.
    job : TransportJob
    ...
    coupler_type = job.wagons[0].coupler_type if job.wagons else CouplerType.SCREW
    yield from loco_service.decouple_wagons(
        popupsim, resource, len(job.wagons), coupler_type
