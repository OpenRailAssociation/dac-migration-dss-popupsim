...
from MVP.popup_retrofit.domain.entities.retrofit_bay import (
    RetrofitBay,
)
from MVP.popup_retrofit.domain.value_objects.popup_metrics import (
    PopUpMetrics,
)
...
class PopUpWorkshop(BaseModel):
    ...
    def start_operations(self) -> None:
        ...
        if self.status != WorkshopStatus.SETUP:
            msg = f"Workshop {self.workshop_id} is not in setup status"
    ...
    def process_wagon(self, wagon: Wagon, current_time: float = 0.0) -> RetrofitResult:
        ...
        if self.status != WorkshopStatus.OPERATIONAL:
            msg = f"Workshop {self.workshop_id} is not operational"
            raise ValueError(msg)
        ...
        if not available_bay:
            msg = "No available bay for retrofit"
            raise ValueError(msg)
