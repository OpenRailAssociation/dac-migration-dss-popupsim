...
class SpecialistTeam(BaseModel):
    ...
    status: TeamStatus = Field(
        default=TeamStatus.AVAILABLE, description="Current status of the team"
    )
    current_bay_id: str | None = Field(
        default=None, description="ID of bay team is working on"
    )

    def assign_to_bay(self, bay_id: str) -> None:
        """Assign team to work on a specific bay."""
        if self.status != TeamStatus.AVAILABLE:
            msg = f"Team {self.team_id} is not available"
