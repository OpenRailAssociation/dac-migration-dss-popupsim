...
from MVP.analytics.domain.exceptions import (
    KPICalculationError,
    MetricsCollectionError,
)
from MVP.analytics.domain.factories.analytics_factory import (
    AnalyticsFactory,
)
from MVP.analytics.domain.models.bottleneck_config import (
    BottleneckConfig,
)
...
from MVP.analytics.domain.specifications.bottleneck_specifications import (
...
class KPICalculator:  # pylint: disable=too-few-public-methods
    ...
    async def calculate_from_simulation(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-locals  # noqa: PLR0913
        ...
        try:
            ...
            msg = f"Failed to calculate KPIs for scenario {scenario.id}: {e}"
            raise KPICalculationError(msg) from e
    ...
    def _calculate_utilization(
        self, workshops: list[Workshop], wagons: list[Wagon]
    ) -> list[UtilizationKPI]:
    ...
    def _identify_bottlenecks(
        ...
        if high_rejection_spec.is_satisfied_by(throughput):
            rejection_rate = (
                throughput.total_wagons_rejected / throughput.total_wagons_processed
            )
            bottlenecks.append(
        ...
        for util in utilization:
            if high_utilization_spec.is_satisfied_by(util):
                severity = (
                    "critical"
                    if critical_utilization_spec.is_satisfied_by(util)
                    else "high"
                )
                bottlenecks.append(
                    AnalyticsFactory.create_bottleneck_info(
    ...
    def _calculate_avg_waiting_time(self, wagons: list[Wagon]) -> float:
        ...
        if not waiting_times:
            return 0.0
        return round(
            sum(waiting_times) / len(waiting_times) / 60.0, 1
        )  # Convert to minutes
    ...
    async def _collect_popup_metrics_async(self, popup_context: Any) -> dict[str, Any]:
        ...
        try:
            if hasattr(popup_context, "get_all_workshop_metrics_async"):
                metrics: dict[
                    str, Any
                ] = await popup_context.get_all_workshop_metrics_async()
            else:
                metrics = dict(popup_context.get_all_workshop_metrics())
            return metrics
        except (AttributeError, TypeError, KeyError) as e:
            msg = f"Failed to collect PopUp metrics: {e}"
            raise MetricsCollectionError(msg) from e
    ...
    async def _collect_yard_metrics_async(self, yard_context: Any) -> dict[str, Any]:
        ...
        try:
            ...
            msg = f"Failed to collect Yard metrics: {e}"
            raise MetricsCollectionError(msg) from e
    ...
    def _collect_shunting_metrics(self, shunting_context: Any) -> dict[str, Any]:
        ...
        try:
            ...
            msg = f"Failed to collect Shunting metrics: {e}"
            raise MetricsCollectionError(msg) from e
    ...
    async def _collect_all_context_metrics_async(
        ...
        popup_task = (
            self._collect_popup_metrics_async(popup_context) if popup_context else None
        )
        yard_task = (
            self._collect_yard_metrics_async(yard_context) if yard_context else None
        )
        shunting_task = (
            self._collect_shunting_metrics_async(shunting_context)
            if shunting_context
            else None
        )
        ...
        tasks = [
            task for task in [popup_task, yard_task, shunting_task] if task is not None
        ]
        if tasks:
            try:
                ...
                msg = f"Failed to collect context metrics: {e}"
                raise MetricsCollectionError(msg) from e
        else:
        ...
        return ContextMetrics(
            ...
            shunting_metrics=shunting_metrics
            if isinstance(shunting_metrics, dict)
            else {},
        )
    ...
    async def _collect_shunting_metrics_async(
        self, shunting_context: Any
    ) -> dict[str, Any]:
        ...
        try:
            if hasattr(shunting_context, "get_shunting_metrics_async"):
                metrics: dict[
                    str, Any
                ] = await shunting_context.get_shunting_metrics_async()
            else:
