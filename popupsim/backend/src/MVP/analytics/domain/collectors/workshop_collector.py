...
class WorkshopCollector(MetricCollector):
    ...
    def record_event(self, event: DomainEvent) -> None:
        ...
        if isinstance(event, WorkshopUtilizationChangedEvent):
            ...
            if workshop_id in self.last_event:
                ...
                if last_stations > 0:
                    self.active_time[workshop_id] = (
                        self.active_time.get(workshop_id, 0.0) + duration
                    )
                else:
                    self.idle_time[workshop_id] = (
                        self.idle_time.get(workshop_id, 0.0) + duration
                    )
    ...
    def get_results(self) -> list[MetricResult]:
        ...
        for workshop_id in set(
            list(self.active_time.keys()) + list(self.idle_time.keys())
        ):
