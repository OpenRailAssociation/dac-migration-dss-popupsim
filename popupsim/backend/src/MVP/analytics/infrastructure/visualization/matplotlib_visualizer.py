...
mpl.use("Agg")  # Use non-interactive backend for headless environments
    ...
    def generate_utilization_chart(self, result: KPIResult, output_path: Path) -> None:
        ...
        if not result.utilization:
            ...
            ax.text(
                0.5,
                0.5,
                "No workshop utilization data available",
                ha="center",
                va="center",
                fontsize=14,
            )
        ...
        avg_utilization = [
            util.average_utilization_percent for util in result.utilization
        ]
        peak_utilization = [
            util.peak_utilization_percent for util in result.utilization
        ]
