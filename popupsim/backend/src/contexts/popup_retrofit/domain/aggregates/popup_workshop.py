...
from contexts.popup_retrofit.domain.value_objects.popup_metrics import (
    PopUpMetrics,
)
...
class PopUpWorkshop(BaseModel):
    ...
    workshop_id: WorkshopId = Field(
        description="Unique identifier for the PopUp workshop"
    )
    ...
    metrics: PopUpMetrics = Field(
        default_factory=PopUpMetrics, description="Workshop performance metrics"
    )
    ...
    def process_wagon(
        self, wagon_id: str, current_time: float = 0.0
    ) -> tuple[RetrofitResult, list[Any]]:
        ...
        # Find available bay
        available_bay = next(
            (bay for bay in self.retrofit_bays if bay.is_available()), None
        )

        if not available_bay:
            return RetrofitResult.failed(
                wagon_id=wagon_id, reason="No available bay for retrofit"
            ), []
