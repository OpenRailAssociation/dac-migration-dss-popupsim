...
from contexts.yard_operations.domain.services.hump_yard_service import (
    HumpYardService,
    YardConfiguration,
    YardType,
)
from contexts.yard_operations.domain.services.rake_transport_service import (
    RakeTransportService,
)
from contexts.yard_operations.domain.services.wagon_distribution_service import (
    WagonDistributionService,
)
from contexts.yard_operations.domain.services.wagon_pickup_service import (
    WagonPickupService,
)
...
class YardOperationsContext:
    ...
    def initialize(self, infrastructure: Any, scenario: Any) -> None:
        ...
        if railway_context:
            self.railway_capacity_service = RailwayCapacityService(railway_context)
        ...
        if railway_context and collection_tracks:
            collection_capacity = railway_context.get_track_capacity(collection_tracks[0].id)
    ...
    def start_processes(self) -> None:
        ...
        self.infra.event_bus.subscribe(
            WagonsClassifiedEvent, self._handle_wagons_classified_for_rakes
        )
    ...
    def _execute_transport_job(self, wagons: list[Any]) -> Generator[Any, Any]:
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, loco.current_track, "collection"
            )

            # Couple wagons
            yield from shunting_context.couple_wagons(self, len(wagons), "SCREW")

            # Select retrofit track and move to retrofit area
            ...
            yield from shunting_context.move_locomotive(
                self, loco, "collection", retrofit_track_id
            )
            ...
            # Return to parking
            yield from shunting_context.move_locomotive(
                self, loco, retrofit_track_id, loco.home_track
            )

            # Update wagon status
            for wagon in wagons:
                wagon.status = "READY_FOR_RETROFIT"
                wagon.track = retrofit_track_id

        finally:
            # Release locomotive
            yield from shunting_context.release_locomotive(self, loco)

        # Second transport job: retrofit to workshop
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco2, loco2.current_track, retrofit_track_id
            )
            ...
            available_workshops = ["WS1"]
            if (
                self.scenario
                and hasattr(self.scenario, "workshops")
                and len(self.scenario.workshops) > 1
            ):
                available_workshops = [w.id for w in self.scenario.workshops]

            # For multi-workshop scenarios, distribute wagons across workshops
            if len(available_workshops) > 1:
                yield from self._distribute_to_multiple_workshops(
                    wagons, available_workshops, loco2, shunting_context
                )
            else:
                yield from self._distribute_to_single_workshop(
                    wagons, "WS1", loco2, shunting_context
                )

            # Return to parking (will be handled by distribution methods)

        finally:
            # Release locomotive
            yield from shunting_context.release_locomotive(self, loco2)

    def _group_wagons_by_retrofit_track(
        self, wagons: list[Any]
    ) -> dict[str, list[Any]]:
    ...
    def _handle_train_arrived(self, event) -> None:
        ...
        if not hasattr(self, "all_wagons"):
            self.all_wagons = []
        ...
        self.infra.engine.schedule_process(
            self._classify_train_wagons(event.wagons, event.train_id)
        )

    def _classify_train_wagons(self, wagons, train_id) -> Any:
        ...
        schedule = self.hump_yard_service.calculate_processing_schedule(
            len(wagons), process_times
        )
        ...
        classification_result = self.hump_yard_service.classify_wagons(
            wagons, self.yard_config, collection_track_id
        )
        ...
        if classification_result.rejected_wagons:
            ...
            try:
                ...
                for wagon in classification_result.rejected_wagons:
                    ...
                    plog.log(f"REJECTED: Wagon {wagon.id} from {train_id} at {current_time:.1f}min - {reason_text}", sim_time=current_time)
            except RuntimeError:
                pass
        ...
        departure_event = TrainDepartedEvent(
            train_id=train_id, departure_time=current_time, wagon_count=len(wagons)
        )
    ...
    def _handle_wagon_retrofit_completed(self, event) -> None:
        ...
        if not self._pending_transport_checks.get(event.workshop_id, False):
            ...
            self.infra.engine.schedule_process(
                self._check_and_transport_batch(event.workshop_id)
            )

    def _get_workshop_capacity_for_batching(self, workshop_id: str) -> int:
        """Get workshop capacity for determining batch size."""
        if self.scenario and hasattr(self.scenario, "workshops") and self.scenario.workshops:
    ...
    def _check_and_transport_batch(self, workshop_id: str) -> Generator[Any, Any]:
        ...
        if wagons_to_transport:
            self._form_and_transport_retrofitted_rake(
    ...
    def _bring_next_batch_from_retrofit(
        self, wagons: list[Any], workshop_id: str
    ) -> Generator[Any, Any]:
        ...
        yield from self._transport_wagon_batch_to_workshop(
            wagons, workshop_id, shunting_context
        )
    ...
    def _pickup_retrofitted_batch(self, wagons: list[Any]) -> Any:
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, loco.current_track, "WS1"
            )
            ...
            yield from shunting_context.move_locomotive(
                self, loco, "WS1", retrofitted_track_id
            )
            ...
            # Move to parking
            yield from shunting_context.move_locomotive(
                self, loco, retrofitted_track_id, loco.home_track
            )
    ...
    def _pickup_individual_wagon(self, wagons: list[Any], workshop_id: str) -> Any:
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, loco.current_track, workshop_id
            )
            ...
            yield from shunting_context.move_locomotive(
                self, loco, workshop_id, retrofitted_track_id
            )
            ...
            for wagon in wagons:
                ...
                if hasattr(wagon, '_workshop_resource') and hasattr(wagon, '_workshop_request'):
                    wagon._workshop_resource.release(wagon._workshop_request)
    ...
    def _move_wagon_to_workshop(self, wagon: Any, workshop_id: str) -> Any:
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, loco.current_track, retrofit_track_id
            )
            ...
            # Move to workshop
            yield from shunting_context.move_locomotive(
                self, loco, retrofit_track_id, workshop_id
            )
            ...
            # Return to parking
            yield from shunting_context.move_locomotive(
                self, loco, workshop_id, loco.home_track
            )
    ...
    def _pickup_and_process_next_batch(
        self, completed_wagons: list[Any], workshop_id: str
    ) -> Any:
        """Combined pickup of completed wagons and processing of next batch to eliminate timing gaps."""
        shunting_context = getattr(self.infra, "shunting_context", None)
        if not shunting_context:
            return
        ...
        try:
            ...
            yield from shunting_context.couple_wagons(
                self, len(completed_wagons), "DAC"
            )

            yield from shunting_context.move_locomotive(
                self, loco, workshop_id, "retrofitted"
            )

            yield from shunting_context.decouple_wagons(
                self, len(completed_wagons), "DAC"
            )
            ...
            if hasattr(self, "_waiting_wagons") and self._waiting_wagons:
                ...
                if len(self.scenario.workshops) > 1:
                    workshop_wagons = [
                        w for w in self._waiting_wagons if w.workshop_id == workshop_id
                    ]
                else:
                    workshop_wagons = self._waiting_wagons[:]

                if workshop_wagons:
                    workshop_capacity = len(
                        completed_wagons
                    )  # Same capacity as completed batch
                    ...
                    for wagon in next_batch:
                        if wagon in self._waiting_wagons:
                            self._waiting_wagons.remove(wagon)
                    ...
                    yield from shunting_context.move_locomotive(
                        self, loco, loco.home_track, retrofit_track_id
                    )

                    yield from shunting_context.couple_wagons(
                        self, len(next_batch), "SCREW"
                    )
                    ...
                    for wagon in next_batch:
                        wagon.track = workshop_id
                        wagon.batch_id = batch_id
                        event = WagonReadyForRetrofitEvent(
                            wagon=wagon, workshop_id=workshop_id
                        )
                        self.infra.event_bus.publish(event)
                    ...
                    yield from shunting_context.move_locomotive(
                        self, loco, loco.home_track, retrofitted_track_id
                    )
    ...
    def _transport_retrofitted_rake(self, rake: Any) -> Generator[Any, Any]:
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, loco.current_track, rake.formation_track
            )
            yield from shunting_context.couple_wagons(self, rake.wagon_count, "DAC")
            yield from shunting_context.move_locomotive(
                self, loco, rake.formation_track, retrofitted_track_id
            )
        ...
        # Track wagons on retrofitted track for batching
        if not hasattr(self, '_retrofitted_accumulator'):
        ...
        has_more_wagons = (
            workshop_id in self._waiting_at_retrofit
            and self._waiting_at_retrofit[workshop_id]
        )
        ...
        if should_move_to_parking:
            ...
            total_length = sum(getattr(w, 'length', 15.0) for w in wagons_to_park)
            ...
            try:
                ...
                yield from shunting_context.move_locomotive(
                    self, loco2, loco2.current_track, retrofitted_track_id
                )
                yield from shunting_context.couple_wagons(self, len(wagons_to_park), "DAC")
                yield from shunting_context.move_locomotive(
                    self, loco2, retrofitted_track_id, parking_track_id
                )
                ...
                for wagon in wagons_to_park:
                    wagon.status = "PARKED"
                    wagon.track = parking_track_id
                    parked_event = WagonParkedEvent(
                        wagon_id=wagon.id, parking_area_id=parking_track_id
                    )
                    parked_event.event_timestamp = current_time
                    self.infra.event_bus.publish(parked_event)
                
                # Update track occupancy
                self._track_occupancy_m[parking_track_id] = self._track_occupancy_m.get(parking_track_id, 0.0) + total_length
                
                yield from shunting_context.move_locomotive(
                    self, loco2, parking_track_id, loco2.home_track
                )
            finally:
                yield from shunting_context.release_locomotive(self, loco2)
        ...
        if (
            ...
            self._waiting_at_retrofit[workshop_id] = self._waiting_at_retrofit[
                workshop_id
            ][next_batch_size:]
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        wagons_on_retrofitted = sum(1 for w in self.wagons if getattr(w, 'track', '') == 'retrofitted')
    ...
    def _calculate_track_utilization(self) -> dict[str, float]:
        ...
        for track_id, queue in self.collection_queues.items():
            if hasattr(queue, '_items'):
                ...
                avg_occupancy = len(queue._items) if queue._items else 0
    ...
    def _transport_rake_to_workshop(self, rake: Any) -> Generator[Any, Any]:
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, rake.formation_track, rake.target_track
            )
            yield from shunting_context.decouple_wagons(self, rake.wagon_count, "SCREW")
            yield from shunting_context.move_locomotive(
                self, loco, rake.target_track, loco.home_track
            )
        finally:
            yield from shunting_context.release_locomotive(self, loco)
        ...
        workshop_ids = [w.id for w in self.scenario.workshops] if self.scenario and hasattr(self.scenario, "workshops") else ["WS1"]
        
        # Log rake wagons before distribution
        wagon_ids = ', '.join([w.id for w in rake.wagons])
        print(f"DEBUG: DISTRIBUTING {len(rake.wagons)} wagons [{wagon_ids}]")
        ...
        # Initialize workshop tracking
        if not hasattr(self, '_workshop_round_robin_index'):
            self._workshop_round_robin_index = 0
        if not hasattr(self, '_workshop_wagon_counts'):
            self._workshop_wagon_counts = {ws_id: 0 for ws_id in workshop_ids}
        ...
        for wagon in rake.wagons:
            if strategy == "round_robin":
                ...
                workshop_id = min(self._workshop_wagon_counts.keys(), 
                                 key=lambda ws: self._workshop_wagon_counts[ws])
        ...
        # Log workshop assignments
        for ws_id, ws_wagons in workshop_batches.items():
            if ws_wagons:
                wagon_ids = ', '.join([w.id for w in ws_wagons])
                print(f"DEBUG: ASSIGNED {len(ws_wagons)} wagons to {ws_id} [{wagon_ids}]")
        ...
        for workshop_id in workshop_ids:
            ...
            # Log batch split
            first_ids = ', '.join([w.id for w in first_batch])
            print(f"DEBUG: FIRST BATCH {len(first_batch)} wagons to {workshop_id} [{first_ids}]")
            if remaining:
                remaining_ids = ', '.join([w.id for w in remaining])
                print(f"DEBUG: WAITING {len(remaining)} wagons for {workshop_id} [{remaining_ids}]")
            
            # Transport first batch
            yield from self._transport_wagon_batch_to_workshop(
                first_batch, workshop_id, shunting_context
            )
    ...
    def _direct_rake_to_workshop(self, rake: Any) -> Generator[Any, Any]:
        ...
        for wagon in self.wagons:
            if hasattr(wagon, "rake_id") and wagon.rake_id == rake.rake_id:
                ...
                event = WagonReadyForRetrofitEvent(
                    wagon=wagon, workshop_id=rake.target_track
                )
                self.infra.event_bus.publish(event)
    ...
    def _transport_wagon_batch_to_workshop(
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, retrofit_track_id, workshop_id, wagon_ids
            )
    ...
    def _get_workshop_capacities(self) -> dict[str, int]:
        ...
        if self.scenario and hasattr(self.scenario, "workshops"):
            for workshop in self.scenario.workshops:
                workshop_capacities[workshop.id] = getattr(
                    workshop, "retrofit_stations", 1
                )
    ...
    def _get_retrofit_track_capacities(self) -> dict[str, float]:
        ...
        if self.railway_capacity_service:
            ...
            if self.scenario and hasattr(self.scenario, "tracks"):
                track_names = [
                    t.id for t in self.scenario.tracks if "retrofit" in t.id.lower()
                ]

            for track_id in track_names:
                capacity_info = self.railway_capacity_service.get_capacity_info(
                    track_id
                )
    ...
    def _select_track_by_type(self, track_type: str, strategy: str | None = None) -> Any | None:
        ...
        if strategy == "least_occupied":
            ...
            if track_type == "parking":
                ...
                available_tracks = [(t, avail_usable) for t, avail_usable, avail_abs, occ, tlen in track_info if avail_usable > 0]
                if available_tracks:
                ...
                tracks_with_absolute_capacity = [(t, avail_abs) for t, avail_usable, avail_abs, occ, tlen in track_info if avail_abs > 0]
    ...
    def _execute_pickup_plan(self, pickup_plan: Any, from_track_id: str) -> Generator[Any, Any]:
        ...
        wagon_ids = ', '.join([w.id for w in all_wagons])
        print(f"DEBUG: EXECUTE_PICKUP_PLAN: {len(all_wagons)} wagons to process [{wagon_ids}]")
        ...
        while remaining_wagons:
            ...
            # Log batch formation
            wagon_ids = ', '.join([w.id for w in rake_wagons])
            print(f"DEBUG: BATCH {batch_num}: {len(rake_wagons)} wagons fit ({total_length:.0f}m/{retrofit_capacity:.0f}m) [{wagon_ids}]")
            if len(remaining_wagons) > len(rake_wagons):
                waiting_ids = ', '.join([w.id for w in remaining_wagons[len(rake_wagons):]])
                print(f"DEBUG: REMAINING: {len(remaining_wagons) - len(rake_wagons)} wagons on collection [{waiting_ids}]")
            batch_num += 1

            remaining_wagons = remaining_wagons[len(rake_wagons) :]
            print(f"DEBUG: After batch {batch_num-1}, {len(remaining_wagons)} wagons remaining")
            ...
            rake = self.wagon_pickup_service.create_single_retrofit_rake(
                ...
                {
                    ws: [
                        w for w in rake_wagons if getattr(w, "workshop_id", None) == ws
                    ]
                    for ws in workshop_allocations.keys()
                },
            )
            ...
            # Log rake formation
            wagon_ids = ', '.join([w.id for w in rake_wagons])
            print(f"DEBUG: RAKE FORMED with {len(rake_wagons)} wagons [{wagon_ids}]")
    ...
    def _get_retrofit_track_capacity(self) -> float:
        ...
        if not self.railway_context:
            raise RuntimeError(
                f"CONFIGURATION ERROR: Railway context not initialized."
            )
    ...
    def _execute_retrofit_track_transport(
        self, wagons: list[Any]
    ) -> Generator[Any, Any]:
        ...
        retrofit_rakes = self._rake_formation_service.form_retrofit_track_rakes(
            wagons, retrofit_tracks, "collection"
        )
    ...
    def _flush_retrofitted_to_parking(self) -> Generator[Any, Any]:
        ...
        try:
            ...
            yield from shunting_context.move_locomotive(
                self, loco, loco.current_track, retrofitted_track_id
            )
            yield from shunting_context.couple_wagons(self, len(wagons_to_park), "DAC")
            yield from shunting_context.move_locomotive(
                self, loco, retrofitted_track_id, parking_track_id
            )
            yield from shunting_context.decouple_wagons(self, len(wagons_to_park), "DAC")
            ...
            yield from shunting_context.move_locomotive(
                self, loco, parking_track_id, loco.home_track
            )
        finally:
            yield from shunting_context.release_locomotive(self, loco)
    ...
    def on_simulation_ended(self, event: Any) -> None:
        """Handle simulation ended - flush remaining wagons."""
        if hasattr(self, '_retrofitted_accumulator') and self._retrofitted_accumulator:
            self.infra.engine.schedule_process(self._flush_retrofitted_to_parking())
