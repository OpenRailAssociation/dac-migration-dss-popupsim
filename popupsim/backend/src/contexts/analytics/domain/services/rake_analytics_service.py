...
class RakeAnalyticsService:
    ...
    def __init__(self) -> None:
        ...
        self.track_occupancy: dict[
            str, list[tuple[float, int]]
        ] = {}  # track -> [(time, wagon_count)]

    def record_rake_formation(
        self, timestamp: float, rake: Rake, strategy: str, workshop_capacity: int = 0
    ) -> None:
    ...
    def record_rake_transport(
        self, timestamp: float, rake: Rake, from_track: str, to_track: str
    ) -> None:
    ...
    def _update_track_occupancy(
        self, track: str, timestamp: float, wagon_delta: int
    ) -> None:
        ...
        current_occupancy = (
            self.track_occupancy[track][-1][1] if self.track_occupancy[track] else 0
        )
    ...
    def get_rake_timeline_for_track(self, track: str) -> list[RakeSnapshot]:
        ...
        track_snapshots = [
            snapshot for snapshot in self.rake_snapshots if snapshot.track == track
        ]
