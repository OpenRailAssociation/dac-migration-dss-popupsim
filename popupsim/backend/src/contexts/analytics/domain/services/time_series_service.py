...
class TimeSeriesService:
    ...
    def get_time_series(
        ...
        while current_time <= end_time:
            ...
            bucket_events = [
                e for ts, e in self.events if current_time <= ts < bucket_end
            ]
    ...
    def _calculate_workshop_utilization(self, events: list[Any]) -> dict[str, int]:
        ...
        completed = sum(
            1 for e in events if type(e).__name__ == "RetrofitCompletedEvent"
        )
    ...
    def get_all_time_series(
        ...
        return {
            metric: self.get_time_series(metric, interval_seconds) for metric in metrics
        }
