...
class TrainArrivalMetrics:
    ...
    total_trains: int
    total_wagons: int
    arrivals_by_time: list[tuple[float, dict[str, int]]]  # (timestamp, {trains, wagons})
...
class MetricsService:
    ...
    def get_train_arrival_metrics(self, interval_seconds: float = 3600.0) -> TrainArrivalMetrics:
        ...
        """
        train_events = [(ts, e) for ts, e in self.events if type(e).__name__ == 'TrainArrivedEvent']

        total_trains = len(train_events)
        total_wagons = sum(len(getattr(e, 'wagons', [])) for _, e in train_events)

        # Time-series aggregation
        arrivals_by_time = self._aggregate_train_arrivals(train_events, interval_seconds)
    ...
    def get_wagon_state_metrics(self) -> WagonStateMetrics:
        ...
        """
        retrofitted = self.event_counts.get('RetrofitCompletedEvent', 0) + self.event_counts.get(
    ...
    def get_locomotive_metrics(self, interval_seconds: float = 3600.0) -> LocomotiveMetrics:
    ...
    def get_track_state_metrics(self, interval_seconds: float = 3600.0) -> list[TrackStateMetrics]:
        ...
        for track_id, track_data in track_occupancy.items():
            ...
            occupancy_over_time = self._calculate_track_occupancy_over_time(track_id, interval_seconds)
    ...
    def _aggregate_train_arrivals(
        ...
        while current_time <= end_time:
            ...
            bucket_events = [e for ts, e in train_events if current_time <= ts < bucket_end]

            trains_count = len(bucket_events)
            wagons_count = sum(len(getattr(e, 'wagons', [])) for e in bucket_events)

            time_series.append((current_time, {'trains': trains_count, 'wagons': wagons_count}))
    ...
    def _calculate_track_occupancy_over_time(self, track_id: str, interval_seconds: float) -> list[tuple[float, float]]:
        ...
        while current_time <= end_time:
            bucket_end = current_time + interval_seconds
            bucket_events = [e for ts, e in track_events if current_time <= ts < bucket_end]
