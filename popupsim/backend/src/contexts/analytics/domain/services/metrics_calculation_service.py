...
class MetricsCalculationService:  # pylint: disable=too-few-public-methods
    ...
    def _calculate_flow(self) -> dict[str, Any]:
        ...
        retrofits_completed = self.event_counts.get('WagonRetrofitCompletedEvent', 0) + self.event_counts.get(
    ...
    def _calculate_throughput(self) -> float:
        ...
        retrofits = self.event_counts.get('WagonRetrofitCompletedEvent', 0) + self.event_counts.get(
            'RetrofitCompletedEvent', 0
        )
    ...
    def _calculate_shunting(self) -> dict[str, Any]:
        ...
        operations_completed = self.event_counts.get('ShuntingOperationCompletedEvent', 0)

        successful_ops = sum(
            1
            for _, e in self.events
            if type(e).__name__ == 'ShuntingOperationCompletedEvent' and getattr(e, 'success', True)
        )
