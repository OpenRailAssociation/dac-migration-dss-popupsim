...
class ExternalTrainsCollector(ContextCollector):
    ...
    def _handle_train_arrived(self, event: Any) -> None:
        ...
        self.collector.record_metric(
            "wagons_arrived", current_wagons + len(event.wagons)
        )
...
class PopupRetrofitCollector(ContextCollector):
    ...
    def _subscribe_to_events(self) -> None:
        ...
        self.event_bus.subscribe(
            WagonReadyForRetrofitEvent, self._handle_retrofit_started
        )
        self.event_bus.subscribe(
            WagonRetrofitCompletedEvent, self._handle_retrofit_completed
        )
...
class MultiContextCollectorCoordinator:
    ...
    def __post_init__(self) -> None:
        ...
        self.collectors = {
            "external_trains": ExternalTrainsCollector(
                "external_trains", self.event_bus
            ),
    ...
    def get_all_metrics(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            context_name: collector.collector.get_all_latest()
            for context_name, collector in self.collectors.items()
        }
