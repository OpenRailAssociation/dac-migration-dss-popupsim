...
from contexts.analytics.domain.services.metrics_query_service import MetricsQueryService
...
class MetricsOutputService:
    ...
    def export_to_csv(self, output_dir: Path) -> None:
        ...
        with bottleneck_file.open('w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['resource_type', 'resource_id', 'utilization_%', 'severity'])
