...
class JSONExporter:
    ...
    def export_comprehensive_report(
        ...
        report_data = {
            ...
            "data": {
                ...
                "kpi_summary": self._sanitize_data(
                    analytics_context.get_advanced_kpis()
                ),
                "cross_context_analysis": self._sanitize_data(
                    analytics_context.get_cross_context_analysis()
                ),
                "real_time_status": self._sanitize_data(
                    analytics_context.get_real_time_status()
                ),
            },
        }
    ...
    def export_dashboard_data(self, analytics_context: Any, output_path: Path) -> None:
        ...
        dashboard_data = {
            ...
            "dashboard": {
                "operational_metrics": self._extract_operational_metrics(
                    analytics_context
                ),
    ...
    def export_time_series(
        self, time_series_data: dict[str, list[tuple[float, Any]]], output_path: Path
    ) -> None:
    ...
    def _extract_operational_metrics(self, analytics_context: Any) -> dict[str, Any]:
        ...
        return {
            "throughput": {
                ...
                "throughput_rate_per_hour": metrics.get(
                    "throughput_rate_per_hour", 0.0
                ),
            },
            "utilization": {
                "workshops": metrics.get("workshop_statistics", {}).get(
                    "workshops", {}
                ),
    ...
    def _extract_performance_summary(self, analytics_context: Any) -> dict[str, Any]:
        ...
        return {
            "bottlenecks": cross_context.get("bottleneck_analysis", {}).get(
                "bottlenecks", []
            ),
            "flow_efficiency": cross_context.get("flow_analysis", {}).get(
