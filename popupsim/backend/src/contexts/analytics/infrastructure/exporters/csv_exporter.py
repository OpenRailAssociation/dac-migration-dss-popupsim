                ...
                for timestamp, value in time_series:
                    if isinstance(value, dict):
                        for sub_key, sub_value in value.items():
                            writer.writerow(
                                [timestamp, f"{metric_name}_{sub_key}", sub_value]
                            )
                    else:
                        writer.writerow([timestamp, metric_name, value])

    def export_utilization_breakdown(
        self, breakdown_data: dict[str, Any], output_path: Path
    ) -> None:
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            # Write header
            writer.writerow(
                ["resource_type", "resource_id", "action", "time_hours", "percentage"]
            )
            ...
            if "locomotive_breakdown" in breakdown_data:
                ...
                for action, time_val in loco.action_times.items():
                    ...
                    writer.writerow(
                        ["locomotive", "fleet", action, time_val, percentage]
                    )
