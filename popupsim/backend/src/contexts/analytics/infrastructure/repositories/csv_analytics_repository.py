...
class CSVAnalyticsRepository(AnalyticsRepository):
    ...
    def _save_session_metadata(self, session: AnalyticsSession) -> None:
        ...
        if file_exists:
            with open(self._sessions_file, encoding="utf-8") as f:
                reader = csv.DictReader(f)
                rows = [
                    row for row in reader if row["session_id"] != session.session_id
                ]
        ...
        rows.append(
            {
                ...
                "total_events": sum(
                    c.events_processed for c in session.get_all_collectors().values()
                ),
            }
        )
    ...
    def _save_session_metrics(self, session: AnalyticsSession) -> None:
        ...
        if rows:
            with open(metrics_file, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["collector_id", "metric_key", "value", "timestamp"]
                )
                writer.writeheader()
                writer.writerows(rows)
    ...
    def _load_session(self, row: dict[str, str]) -> AnalyticsSession | None:
        ...
        session = AnalyticsSession(
            session_id=row["session_id"], start_time=float(row["start_time"])
        )
