...
class TrainArrivalCoordinator:  # pylint: disable=too-few-public-methods
    ...
    def process_train_arrivals(self, popupsim: Any) -> Generator[Any]:
        ...
        external_trains = self.port.get_arrival_schedule(self.scenario.start_date, self.scenario.end_date)

        if not external_trains:
            raise ValueError('No trains scheduled in scenario')

        logger.info('Starting external train arrival coordinator for scenario %s', self.scenario.id)
        ...
        for external_train in external_trains:
            ...
            delay_minutes = (external_train.arrival_time - self.scenario.start_date).total_seconds() / 60.0
            yield popupsim.sim.delay(delay_minutes)

            # Mark train as arrived
            external_train.mark_arrived()
            logger.info('External train %s arrived from %s', external_train.train_id, external_train.origin_station)

            # Publish ExternalTrainArrivedEvent (future: publish to event bus)
            logger.debug('ExternalTrainArrivedEvent: train=%s origin=%s operator=%s',
                        external_train.train_id, external_train.origin_station, external_train.operator)

            # Mark wagons delivered
            external_train.mark_wagons_delivered()

            # Publish WagonsDeliveredEvent (future: publish to event bus)
            logger.debug('WagonsDeliveredEvent: train=%s wagons=%d location=yard_entry',
                        external_train.train_id, len(external_train.wagons))
