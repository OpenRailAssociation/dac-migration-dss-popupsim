...
app = typer.Typer(
    name="popupsim-new", help="PopUpSim New Architecture - Bounded contexts"
)
...
def run(
    ...
    output_path: Annotated[
        Path, typer.Option("--output", help="Output directory")
    ] = Path("./output"),
    ...
    event_handler = logging.FileHandler(
        output_path / "events.log", mode="w", encoding="utf-8"
    )
    ...
    if result.success:
        ...
        if external_trains:
            ...
            typer.echo(
                f"  Total wagons arrived:     {ext_metrics.get('total_wagons', 0)}"
            )
            typer.echo(
                f"  Wagons completed:         {ext_metrics.get('completed_wagons', 0)}"
            )
        ...
        if popup:
            ...
            typer.echo(f"\nWORKSHOP METRICS:")
            typer.echo(
                f"  Workshops:                {popup_metrics.get('workshops', 0)}"
            )
            typer.echo(
                f"  Total retrofit bays:      {popup_metrics.get('total_bays', 0)}"
            )
            typer.echo(
                f"  Overall utilization:      {popup_metrics.get('utilization_percentage', 0):.1f}%"
            )
        ...
        if yard:
            ...
            typer.echo(f"\nYARD METRICS:")
            typer.echo(
                f"  Wagons classified:        {yard_metrics.get('classified_wagons', 0)}"
            )
            typer.echo(
                f"  Wagons rejected:          {yard_metrics.get('rejected_wagons', 0)}"
            )
            typer.echo(
                f"  Wagons on retrofitted:    {yard_metrics.get('wagons_on_retrofitted', 0)}"
            )
            typer.echo(
                f"  Wagons parked:            {yard_metrics.get('wagons_parked', 0)}"
            )
        ...
        if shunting:
            ...
            typer.echo(f"\nSHUNTING METRICS:")
            typer.echo(
                f"  Total locomotives:        {shunt_metrics.get('total_locomotives', 0)}"
            )
            typer.echo(
                f"  Locomotive utilization:   {shunt_metrics.get('utilization_percentage', 0):.1f}%"
            )
