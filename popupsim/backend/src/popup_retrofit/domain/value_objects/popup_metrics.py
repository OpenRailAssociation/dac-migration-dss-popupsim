...
class PopUpMetrics(BaseModel):
    ...
    workshop_id: str = Field(description='PopUp workshop identifier')
    total_processing_time: float = Field(default=0.0, description='Total time spent processing wagons (minutes)')
    total_wagons_processed: int = Field(default=0, description='Total number of wagons processed')
    total_bay_hours: float = Field(default=0.0, description='Total bay operating hours')
    occupied_bay_hours: float = Field(default=0.0, description='Total hours bays were occupied')
    ...
    def bay_utilization_percentage(self) -> float:
        """Bay utilization as percentage."""
        if self.total_bay_hours == 0:
            return 0.0
        return (self.occupied_bay_hours / self.total_bay_hours) * 100.0

    @property
    def dac_installation_success_rate(self) -> float:
        """DAC installation success rate as percentage (always 100% for now)."""
        return 100.0
    ...
    def wagons_per_hour(self) -> float:
        ...
        if self.total_processing_time == 0:
            return 0.0
        return (self.total_wagons_processed * 60.0) / self.total_processing_time

    def calculate_efficiency_score(self) -> float:
        """Calculate overall PopUp efficiency score (0-100)."""
        utilization_score = min(self.bay_utilization_percentage, 100.0)
        throughput_score = min(self.wagons_per_hour * 10.0, 100.0)  # Scale throughput
