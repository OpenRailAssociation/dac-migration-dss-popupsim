...
from contexts.configuration.application.configuration_context import (
    ConfigurationContext,
)
from contexts.configuration.domain.models.scenario import Scenario
from contexts.external_trains.application.external_trains_context import (
    ExternalTrainsContext,
)
from contexts.popup_retrofit.application.popup_context import PopUpRetrofitContext
from contexts.railway_infrastructure.application.railway_context import (
...
from shared.infrastructure.simulation.coordination.simulation_infrastructure import (
...
class SimulationApplicationService:
    ...
    def execute(self, until: float) -> SimulationResult:
        ...
        try:
            ...
            logger.info(
                " Simulation completed successfully for scenario %s", self.scenario.id
            )
            return result
            ...
            logger.exception(
                " Simulation failed for scenario %s: %s", self.scenario.id, e
            )
            ...
            return SimulationResult(
                metrics={}, duration=failure_time - start_time, success=False
            )
    ...
    def _register_all_contexts(self) -> None:
        ...
        # For backward compatibility, create a finalized scenario from existing scenario
        if self.scenario:
            config_context._finalized_scenarios[self.scenario.id] = self.scenario
        ...
        self.context_registry.register_context(
            "external_trains", external_trains_context
        )
    ...
    def _setup_workshop_infrastructure(self) -> None:
        """Setup workshop infrastructure from scenario configuration."""
        ...
        if self.scenario.workshops:
            for workshop in self.scenario.workshops:
                ...
                if workshop_id not in self.infra.wagons_for_retrofit:
                    self.infra.wagons_for_retrofit[workshop_id] = (
                        self.engine.create_store()
                    )
    ...
    def _orchestrate_simulation(self) -> Any:
        """Main simulation orchestration process."""
