...
class LocomotivePool:
    ...
    Locomotives are managed via SimPy Store allowing blocking get/put operations.
    """

    def __init__(self, sim: SimulationAdapter, locomotives: list[Locomotive]) -> None:
        self.sim = sim
        self.all_locomotives: dict[str, Locomotive] = {loco.locomotive_id: loco for loco in locomotives}
        self.store: Any = sim.create_store(capacity=len(locomotives))  # type: ignore[attr-defined]
...
def pickup_wagons_to_retrofit(popupsim: PopupSim) -> Generator[Any]:
    ...
    while True:
        ...
        if route_to_collection and route_to_collection.duration:
            logger.info(
                '🚂 ROUTE: Loco %s traveling [%s → %s] (duration: %.1f min)',
                loco.locomotive_id,
                loco.track_id,
                collection_track_id,
                route_to_collection.duration,
            )
        ...
        for retrofit_track_id, retrofit_wagons in wagons_by_retrofit.items():
            ...
            if route_to_retrofit and route_to_retrofit.duration:
                logger.info(
                    '🚂 ROUTE: Loco %s traveling [%s → %s] with %d wagons (duration: %.1f min)',
                    loco.locomotive_id,
                    loco.track_id,
                    retrofit_track_id,
                    len(retrofit_wagons),
                    route_to_retrofit.duration,
                )
            ...
            logger.debug(
                'Loco %s decoupling %d wagons to %d stations',
                loco.locomotive_id,
                len(wagons_to_deliver),
                available_stations,
            )
...
def pickup_retrofitted_wagons(popupsim: PopupSim) -> Generator[Any]:
    ...
    while True:
        ...
        if route_to_retrofitted and route_to_retrofitted.duration:
            logger.debug(
                'Loco %s traveling to retrofitted track with %d wagons', loco.locomotive_id, len(wagons_to_pickup)
            )
...
def move_to_parking(popupsim: PopupSim) -> Generator[Any]:
    ...
    while True:
        ...
        for wagon in wagons_to_move:
            ...
            wagon.status = WagonStatus.MOVING
            wagon.source_track_id = retrofitted_track.id
            wagon.destination_track_id = parking_track.id
            wagon.track_id = None
        ...
        if route_to_parking and route_to_parking.duration:
            logger.debug(
                'Loco %s traveling to parking %s with %d wagons',
                loco.locomotive_id,
                parking_track.id,
                len(wagons_to_move),
            )
