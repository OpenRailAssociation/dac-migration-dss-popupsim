...
from shared.domain.services.multi_track_transport_service import (
    MultiTrackTransportService,
)
...
from shared.domain.services.retrofit_track_allocation_service import (
    RetrofitTrackAllocationService,
)
...
class RetrofitTrackStrategy(RakeFormationStrategy):
    ...
    def form_rakes(
        ...
        transport_plan = self.transport_service.create_transport_plan(
            allocation, from_track
        )

        # Validate transport plan
        is_valid, _issues = self.transport_service.validate_transport_plan(
            transport_plan
        )
        if not is_valid:
            pass
        ...
        if allocation.overflow_wagons:
            overflow_rake = self._create_overflow_rake(
                allocation.overflow_wagons, from_track, retrofit_tracks
            )
    ...
    def _create_overflow_rake(
        self, wagons: list[Wagon], from_track: str, retrofit_tracks: dict[str, float]
    ) -> Rake:
