...
class SimPyEngineAdapter(SimulationEnginePort):
    ...
    @classmethod
    def create(cls) -> Self:
        """Factory method to create enhanced adapter."""
    ...
    def schedule_process(self, process: Generator[Any] | Callable) -> Any:
        ...
        try:
            if inspect.isgenerator(process):
                logger.debug(
                    "SIMPY: Scheduling generator %s at t=%.1f", process, self._env.now
                )
            ...
            logger.debug(
                "SIMPY: Wrapping non-generator process at t=%.1f", self._env.now
            )
    ...
    def run(self, until: float | None = None) -> None:
        ...
        logger.info(
            "SIMPY: Starting simulation run (until=%s)", until if until else "inf"
        )
        ...
        try:
            ...
            if "callbacks" in str(e):
                logger.error(
                    "SIMPY: Invalid yield - generator yielded instead of 'yield from'"
                )
    ...
    def create_store(self, capacity: int | None = None, name: str | None = None) -> Any:
        ...
        if capacity is None:
            ...
            store = simpy.Store(self._env, capacity=capacity)
    ...
    def get_store_stats(self, store: Any) -> dict[str, Any]:
        ...
        if hasattr(store, "capacity") and hasattr(store, "items"):
            return {
                ...
                "available_space": store.capacity - len(store.items)
                if store.capacity != float("inf")
                else None,
            }
        return {}
