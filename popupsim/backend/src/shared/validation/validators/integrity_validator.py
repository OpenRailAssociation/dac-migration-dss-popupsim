...
class IntegrityValidator:  # pylint: disable=too-few-public-methods
    ...
    def _validate_train_references(
        ...
        for i, train in enumerate(scenario.trains):
            ...
            if (
                hasattr(train, "locomotive_id")
                and train.locomotive_id not in locomotive_ids
            ):
                result.add_error(
                    ...
                    suggestion=(
                        f"Use one of: {', '.join(locomotive_ids)}"
                        if locomotive_ids
                        else "Add locomotives first"
                    ),
                )
            ...
            if hasattr(train, "route_id") and train.route_id not in route_ids:
                result.add_error(
                    ...
                    category=ValidationCategory.INTEGRITY,
                    suggestion=(
                        f"Use one of: {', '.join(route_ids)}"
                        if route_ids
                        else "Add routes first"
                    ),
                )

    def _validate_workshop_references(
        self, scenario: Scenario, result: ValidationResult, track_ids: set[str]
    ) -> None:
        ...
        for i, workshop in enumerate(scenario.workshops):
            if hasattr(workshop, "track") and workshop.track not in track_ids:
                result.add_error(
                    ...
                    category=ValidationCategory.INTEGRITY,
                    suggestion=(
                        f"Use one of: {', '.join(track_ids)}"
                        if track_ids
                        else "Add tracks first"
                    ),
                )
    ...
    def _validate_train_time_consistency(
        self, scenario: Scenario, result: ValidationResult
    ) -> None:
        ...
        for i, train in enumerate(scenario.trains):
            if not (
                hasattr(train, "arrival_time") and hasattr(train, "departure_time")
            ):
                continue
            try:
