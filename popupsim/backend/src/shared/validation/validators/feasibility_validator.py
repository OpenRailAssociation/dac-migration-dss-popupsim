...
class FeasibilityValidator:  # pylint: disable=too-few-public-methods
    ...
    def _validate_capacity_constraints(self, scenario: Scenario, result: ValidationResult) -> None:
        """Validate capacity vs demand constraints."""
        if not scenario.workshops or not scenario.trains:
            return

        # Calculate total workshop capacity
        total_retrofit_capacity = sum(getattr(w, 'retrofit_stations', 0) for w in scenario.workshops)
    ...
    def _validate_operational_feasibility(self, scenario: Scenario, result: ValidationResult) -> None:
    ...
    def _validate_simulation_readiness(self, scenario: Scenario, result: ValidationResult) -> None:
