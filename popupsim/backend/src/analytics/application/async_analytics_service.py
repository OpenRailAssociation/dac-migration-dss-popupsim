...
from analytics.domain.models.simulation_data import ContextData
from analytics.domain.models.simulation_data import SimulationData
...
class AsyncAnalyticsService:
    ...
    async def calculate_kpis_async(
        ...
        logger.info('Starting async KPI calculation for scenario %s', simulation_data.scenario.id)
        ...
        try:
            ...
            logger.info('Completed async KPI calculation for scenario %s', simulation_data.scenario.id)
            return result
        except Exception as e:
            logger.error('Async KPI calculation failed for scenario %s: %s', simulation_data.scenario.id, e)
            raise KPICalculationError(f'Async KPI calculation failed for scenario {simulation_data.scenario.id}') from e

    def start_background_calculation(
        self, simulation_data: SimulationData, context_data: ContextData | None = None
    ) -> asyncio.Task:
        """Start KPI calculation in background, return immediately."""
        task = asyncio.create_task(self.calculate_kpis_async(simulation_data, context_data))
