...
class YardMetrics(BaseModel):
    ...
    total_wagons_processed: int = Field(default=0, description='Total wagons processed through hump')
    total_wagons_classified: int = Field(default=0, description='Total wagons successfully classified')
    total_wagons_rejected: int = Field(default=0, description='Total wagons rejected at hump')
    total_hump_time: float = Field(default=0.0, description='Total hump processing time (minutes)')
    rejection_stats: RejectionStats = Field(default_factory=RejectionStats, description='Detailed rejection statistics')
    peak_rejection_rate: float = Field(default=0.0, description='Peak rejection rate during simulation')
    ...
    def get_bottleneck_analysis(self) -> str:
        ...
        if self.hump_throughput_per_hour < 30.0:
            return f'Low hump throughput: {self.hump_throughput_per_hour:.1f} wagons/hour'
