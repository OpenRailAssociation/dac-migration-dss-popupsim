    ...
    def find_wagons_for_retrofit(
        ...
        for wagon in collection_wagons:
            for retrofit_track in retrofit_tracks:
                ...
                if has_workshop_capacity and self.track_capacity.can_add_wagon(retrofit_track_id, wagon.length):
                    wagons_to_pickup.append((wagon, retrofit_track_id))
                    break

        return wagons_to_pickup

    def group_wagons_by_retrofit_track(self, wagons_to_pickup: list[tuple[Wagon, str]]) -> dict[str, list[Wagon]]:
