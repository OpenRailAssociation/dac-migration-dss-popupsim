...
class ParkingArea:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, parking_tracks: list[Track], track_capacity: TrackCapacityManager) -> None:
    ...
    def select_parking_track(self, wagons: list[Wagon]) -> Track | None:
        ...
        for i in range(len(self.parking_tracks)):
            ...
            if any(self.track_capacity.can_add_wagon(track.id, w.length) for w in wagons):
                self.current_track_index = idx
                return track

        return None

    def get_wagons_that_fit(self, track: Track, wagons: list[Wagon]) -> tuple[list[Wagon], list[Wagon]]:
    ...
    def advance_to_next_track(self) -> None:
        ...
        self.current_track_index = (self.current_track_index + 1) % len(self.parking_tracks)
