...
from yard_operations.domain.value_objects.rejection_reason import RejectionReason
from yard_operations.domain.value_objects.rejection_reason import RejectionStats
...
class HumpYardService:  # pylint: disable=too-few-public-methods
    ...
    def process_wagon(
        ...
        if decision == ClassificationDecision.RETROFIT and collection_track_id:
            ...
            rejection_reason = self._determine_rejection_reason(wagon, collection_track_id)
    ...
    def _determine_rejection_reason(self, wagon: Wagon, collection_track_id: str | None) -> RejectionReason:
        ...
        if not self.classification_area.track_capacity.can_add_wagon(collection_track_id, wagon.length):
