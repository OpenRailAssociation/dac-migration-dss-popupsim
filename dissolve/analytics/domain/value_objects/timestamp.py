...
from shared.infrastructure.time_converters import from_ticks, to_ticks
...
class Timestamp:
    ...
    def __post_init__(self) -> None:
        ...
        if self.duration.total_seconds() < 0:
            raise ValueError(f"Timestamp cannot be negative: {self.duration}")
    ...
    def from_datetime(cls, dt: datetime | str, start_time: datetime) -> "Timestamp":
        ...
        if duration.total_seconds() < 0:
            raise ValueError(f"Datetime {dt} is before start time {start_time}")
