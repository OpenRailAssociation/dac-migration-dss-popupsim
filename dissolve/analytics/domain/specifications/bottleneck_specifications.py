...
from ..models.kpi_result import BottleneckInfo, ThroughputKPI, UtilizationKPI
...
class HighRejectionRateSpec(Specification[ThroughputKPI]):
    ...
    def __init__(self, config: BottleneckConfig) -> None:
        self.threshold = (
            config.global_rejection_rate_percent / 100.0
        )  # Convert to decimal
    ...
    def is_satisfied_by(self, candidate: ThroughputKPI) -> bool:
        ...
        if candidate.total_wagons_processed == 0:
            return False
        rejection_rate = (
            candidate.total_wagons_rejected / candidate.total_wagons_processed
        )
