...
class WorkshopCapacityManager:  # pylint: disable=too-few-public-methods
    ...
    def release_stations(
        self, track_id: str, count: int, current_time: float = 0.0
    ) -> None:
        """Release stations (for backward compatibility - will be removed)."""
        for _ in range(count):
            self.record_station_released(track_id, current_time)

    def record_station_occupied(
        self, track_id: str, wagon_id: str, time: float
    ) -> None:
                ...
                if station.last_occupied_time is not None:
                    ...
                    if station.current_wagon_id:
                        station.history.append(
                            (station.last_occupied_time, time, station.current_wagon_id)
                        )
                station.wagons_completed += 1
                station.current_wagon_id = None
                break

    def mark_station_busy(self, track_id: str, wagon_id: str) -> None:
        """Mark station as busy for a wagon."""
        self.record_station_occupied(track_id, wagon_id, self.sim.current_time())
        available = self.get_available_stations(track_id)
        logger.debug(
            "🔧 CAPACITY: Station marked busy for wagon %s at track %s (available=%d)",
            wagon_id,
            track_id,
            available,
        )

    def mark_station_available(self, track_id: str) -> None:
        """Mark station as available."""
        self.record_station_released(track_id, self.sim.current_time())
        available = self.get_available_stations(track_id)
        logger.debug(
            "🔓 CAPACITY: Station marked available at track %s (available=%d)",
            track_id,
            available,
        )
