...
from popup_retrofit.domain.value_objects.retrofit_request import RetrofitRequest

from workshop_operations.domain.entities.wagon import CouplerType, WagonStatus
...
class WorkshopProcessingCoordinator:  # pylint: disable=too-few-public-methods
    ...
    def move_wagons_to_stations(self) -> Generator:
        ...
        for track_id in self.orchestrator.wagons_ready_for_stations:
            self.orchestrator.sim.schedule_process(
                self._process_track_batches(track_id)
            )
    ...
    def _process_track_batches(self, workshop_track_id: str) -> Generator:
        ...
        workshop = self.orchestrator.workshop_capacity.workshops_by_track[
            workshop_track_id
        ]
        batch_size = workshop.retrofit_stations

        while True:
            batch_wagons, retrofit_track_id = yield from self._collect_wagon_batch(
                workshop_track_id, batch_size
            )
            ...
            yield from self._deliver_batch_to_workshop(
                batch_wagons, retrofit_track_id, workshop.track
            )
    ...
    def _collect_wagon_batch(
        ...
        for i in range(batch_size):
            if (
                i > 0
                and len(
                    self.orchestrator.wagons_ready_for_stations[workshop_track_id].items
                )
                == 0
            ):
                break

            item = yield self.orchestrator.wagons_ready_for_stations[
                workshop_track_id
            ].get()
    ...
    def _wait_for_workshop_ready(
        self, workshop_track_id: str, workshop: Workshop
    ) -> Generator:
    ...
    def _deliver_batch_to_workshop(
        ...
        # Allocate locomotive
        loco = yield from self.orchestrator.locomotive_service.allocate(
            self.orchestrator
        )
        ...
        try:
            ...
            yield from self.orchestrator.locomotive_service.move(
                self.orchestrator, loco, loco.track, retrofit_track_id
            )
            ...
            for wagon in batch:
                self.orchestrator.track_capacity.remove_wagon(
            ...
            for wagon in batch:
                self.orchestrator.track_capacity.add_wagon(
                    workshop_track_id, wagon.length
                )
            ...
            yield from self.orchestrator.locomotive_service.move(
                self.orchestrator, loco, loco.track, parking_track_id
            )

        finally:
            # Release locomotive
            yield from self.orchestrator.locomotive_service.release(
                self.orchestrator, loco
            )

    def _process_wagons_at_workshop(
        self, batch: list[Wagon], workshop_track_id: str
    ) -> Generator:
        """Spawn processing for each wagon at workshop."""
        logger.info(
            "🏭 WORKSHOP: Starting processing for batch %s at track %s (t=%.1f)",
            [w.id for w in batch],
            workshop_track_id,
        ...
        for wagon in batch:
            ...
            self.orchestrator.sim.schedule_process(
                self._process_single_wagon(wagon, workshop_track_id)
            )
            logger.debug("  ✓ Scheduled processing for wagon %s", wagon.id)
        yield self.orchestrator.sim.delay(0)
        logger.debug(
            "✅ WORKSHOP: All wagon processes scheduled for track %s", workshop_track_id
        )

    def _process_single_wagon(self, wagon: Wagon, track_id: str) -> Generator:
        """Process single wagon at PopUp workshop."""
        logger.debug(
            "🔧 PROCESS: Starting wagon %s processing at track %s (t=%.1f)",
            wagon.id,
            track_id,
            self.orchestrator.sim.current_time(),
        )
        ...
        yield from station_service.process_wagon_at_station(request)

        # Signal completion
        logger.info(
            "📤 PROCESS: Wagon %s completed, signaling pickup (t=%.1f)",
            wagon.id,
            self.orchestrator.sim.current_time(),
        )
