...
class ShuntingLocomotive:
    ...
    def is_at_capacity(self) -> bool:
        ...
        return (
            ...
            or self.current_load.current_weight_tons
            >= self.capacity_limits.max_weight_tons
            or self.current_load.current_consist_length_m
            >= self.capacity_limits.max_consist_length_m
        )
    ...
    def get_capacity_utilization(self) -> dict[str, float]:
        ...
        return {
            "wagon_utilization": (
                self.current_load.coupled_wagons
                / self.capacity_limits.max_wagon_capacity
            )
            * 100,
            "weight_utilization": (
                self.current_load.current_weight_tons
                / self.capacity_limits.max_weight_tons
            )
            * 100,
            "length_utilization": (
                self.current_load.current_consist_length_m
                / self.capacity_limits.max_consist_length_m
            )
            * 100,
        }
